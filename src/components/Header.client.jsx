...
export default function Header({
  ...
  return (
    ...
    >
      <div
        className={`fixed w-full z-10 cursor-pointer h-full top-0 left-0 ${
          isNavOpen ? '' : 'hidden'
        }`}
    ...
    el = (
      ...
      >
        <Link to="/" className={`logo full-logo mt-2`}>
          <div className={'mx-auto absolute top-0 text-center'}>
            <Logo />
          </div>
        </Link>
    ...
    <>
      <div
        className={`topbar-overlay bg-white w-full h-full absolute ${
          backgroundTransparency ? 'transparent-bg' : ''
        } ${scrollPosition > 100 ? 'show-header' : ''}`}
      ></div>
      <NavBackground />
    </>
  );
