...
import {AddToCartButton, BuyNowButton} from '@shopify/hydrogen';
  ...
  return (
    <div className="space-y-2 mb-8 text-md">
      ...
      {isOutOfStock ? (
        ...
        <BuyNowButton className={`${BUTTON_SECONDARY_CLASSES} py-3 rounded-lg`}>
          Buy it now
        </BuyNowButton>
      )}
    </div>
...
export default function ProductDetails({product, designer}) {
  ...
  const editorsNotesMetafield = product.metafields
    ? product.metafields.filter((m) => m?.key === 'editor_s_notes')[0]
    : null;
  ...
  function getTabs(editorsNotesMetafield) {
    ...
    if (product.description)
      tabs[0] = {
        label: 'DETAILS & CARE',
        component: (
          <div className=" pb-8 pt-4 text-black text-md">
            {product.description}
          </div>
        ),
      };
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-x-0 pb-14 md:pb-20 md:grid-cols-[1fr,1fr] ">
        ...
        <Gallery media={product.media.nodes} />
            ...
            <div className={'pt-12 md:pt-28 pr-7  md:max-w-lg'}>
              <div className=" md:block uppercase">
                ...
                <div className={'flex justify-between items-center'}>
                  <h1 className=" font-semibold pt-3 text-black mb-4">
                    {product.title}
                  </h1>
              ...
              <div className="mt-10 mb-2">
                <ProductOptions />
                <AddToCartMarkup />
                <div className="flex items space-x-4"></div>
              </div>
...
function Designer({designerData, desktop}) {
  ...
  return (
    <section
      className={`pb-16 pt-0 ${desktop ? 'hidden md:block ' : 'md:hidden'}`}
    >
      <h3
        className={
          'text-2xl tracking-widest font-medium mb-2 text-gray-900 mt-8 uppercase'
        }
      >
        Discover {designerData.title}
      </h3>
      ...
      <div
        className={'pt-4 uppercase cursor-pointer flex items-center '}
